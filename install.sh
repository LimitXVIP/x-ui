...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/LimitXVIP/x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}Failed to fetch x-ui version, it maybe due to Github API restrictions, please try it later${plain}"
            exit 1
        fi
        echo -e "Got x-ui latest version: ${last_version}, beginning the installation..."
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}.tar.gz https://github.com/LimitXVIP/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/LimitXVIP/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz"
    ...
    if [[ -e /usr/local/x-ui/ ]]; then
        ...
        if [[ $yn =~ "Y"|"y" ]]; then
            cd
            mv /etc/x-ui/x-ui.db /etc/x-ui-limitx.db.bak # Backing up Chinese X-UI db (if any)
            mv /etc/x-ui-limitx/x-ui-limitx.db /etc/x-ui-limitx.db.bak # Backing up English X-UI db 
            systemctl stop x-ui
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/LimitXVIP/x-ui/main/x-ui.sh
    ...
    echo -e "----------------------------------------------"
    echo -e "LimitX Official   - https://github.com/LimitXVIP"	
}
